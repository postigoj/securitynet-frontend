...
export const getGuardEvents = createAsyncThunk("GUARD_EVENTS", async (id) => {
  try {
    const response = await axios.get(`/api/event/securityguards/${id}`);
...
const guardEventsReducer = createReducer([], {
  [getGuardEvents.fulfilled]: (state, action) => action.payload,
...
export default guardEventsReducer;